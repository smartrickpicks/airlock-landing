...
interface DagNode {
  ...
  description: string; persona: string; is_gate: boolean; platform_hook?: string
}
                ...
                <div className={`flex items-start gap-3 rounded-lg px-2.5 py-2 ${node.is_gate ? 'bg-[#00D1FF]/[0.04] border border-[#00D1FF]/15' : 'hover:bg-[rgba(255,255,255,0.02)]'} transition-colors`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <p className="text-[9px] text-[#7C5CFC]/50 font-mono">Otto → {node.persona}</p>
                      {node.platform_hook && <p className="text-[9px] text-[#00D1FF]/40 font-mono">via {node.platform_hook}</p>}
                    </div>
                  </div>
                </div>
              </motion.div>
...
function formatSpecAsText(spec: DagSpec): string {
  ...
  spec.dag.nodes.forEach((node, i) => {
    ...
    const hook = node.platform_hook ? `\n   Powered by: ${node.platform_hook}` : ''
    text += `${i + 1}. [${node.chamber.toUpperCase()}] ${node.label}${gate}\n   ${node.description}\n   Otto mode: ${node.persona}${hook}\n`
